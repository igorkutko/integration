...
class Exchange(models.TransientModel):
    ...
    system_id = fields.Many2one(
        comodel_name='integration.external_system',
        related='service_id.system_id'
    )
    service_id = fields.Many2one(
        comodel_name='integration.external_service'
    )
    ...
    request_ids = fields.One2many(
        comodel_name='integration.external_request',
        inverse_name='exchange_id'
    )
    jira_issue_ids = fields.One2many(
        comodel_name='integration.jira_issue',
        inverse_name='exchange_id'
    )
    jira_worklog_ids = fields.One2many(
        comodel_name='integration.jira_worklog',
        inverse_name='exchange_id'
    )
    jira_comment_ids = fields.One2many(
        comodel_name='integration.jira_comment',
        inverse_name='exchange_id'
    )
    ...
    def default_get(self, fields_list):
        ...
        vals['service_id'] = self.env.ref(
            'jira.integration_external_service_issue_updated').id
        vals['worklog_service_id'] = self.env.ref(
            'jira.integration_external_service_worklog').id
        return vals
    ...
    def clear_requests(self, issue_id=''):
        ...
        request_manager = self.env['integration.external_request']
    ...
    def clear_data(self):
        ...
        jira_worklog_manager = self.env['integration.jira_worklog']
        jira_worklog_recordset = jira_worklog_manager.search(domain)
        jira_worklog_recordset.unlink()

        jira_comment_manager = self.env['integration.jira_comment']
